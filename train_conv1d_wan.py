...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for model in models:
                if isinstance(unwrap_model(model), MyWanTransformer3DModel):
    ...
    def encode_video(video):
        ...
        latent_dist = DiagonalGaussianDistribution(latents)
        return latent_dist

    train_dataset.instance_videos = [encode_video(video) for video in train_dataset.instance_videos]

    def collate_fn(examples):
        videos = [example["instance_video"].sample() for example in examples]
