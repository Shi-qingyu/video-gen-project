...
import diffusers
from diffusers import DDIMScheduler, TextToVideoSDPipeline, AutoencoderKL, UNet3DConditionModel
...
def log_validation(
    ...
    pipe.scheduler = DDIMScheduler.from_config(pipe.scheduler.config, **scheduler_args)
...
def encode_prompt(
    ...
    prompt = [prompt] if isinstance(prompt, str) else prompt
    prompt_embeds = _get_clip_prompt_embeds(
        tokenizer,
        text_encoder,
        prompt=prompt,
...
def main(args):
    ...
    if args.version == "skipconv1d":
        attn_processor_type = SkipAttnProcessor2_0
    else:
    ...
    for key, value in unet.attn_processors.items():
        ...
        if "down" in key:
            ...
            height = args.height // 8 // (2 ** block_index)
            width = args.width // 8 // (2 ** block_index)
            frames = args.max_num_frames
        elif "up" in key:
            block_index = int(key.split(".")[1])
            dim = model_config.block_out_channels[3 - block_index]
            height = args.height // 8 // (2 ** (3 - block_index))
            width = args.width // 8 // (2 ** ( 3 - block_index))
            frames = args.max_num_frames
        elif "mid" in key:
            height = args.height // 8 // (2 ** 3)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for model in models:
                if isinstance(unwrap_model(model), UNet3DConditionModel):
    ...
    def encode_video(video):
        video = video.to(accelerator.device, dtype=vae.dtype)  # [F, C, H, W]
    ...
    def collate_fn(examples):
        ...
        videos = torch.stack(videos, dim=0) # [B, F, C, H, W]
    ...
    unet_temporal_conv1d_parameters = []
    for name, param in unet.named_parameters():
        if "processor" in name and param.requires_grad:
            assert "temp" in name, f"Unexpected param named {name}"
            unet_temporal_conv1d_parameters.append(param)

    # Optimization parameters
    unet_parameters_with_lr = {"params": unet_temporal_conv1d_parameters, "lr": args.learning_rate}
    ...
    model_config = unet.module.config if hasattr(unet, "module") else unet.config
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                model_input = batch["videos"].permute(0, 2, 1, 3, 4).to(dtype=weight_dtype)  # [B, C, F, H, W]
                ...
                # Predict the noise residual, (batch, channel, framerate, width, height)
                model_pred = unet(
                    sample=noisy_model_input,    # [B, C, F, H, W]
                ...
                if timesteps <= 400:
                    ...
                    if args.high_frequency_loss:
                        ...
                        hf_loss = torch.mean(((hf_target - hf_model_pred) ** 2).reshape(batch_size, -1), dim=1)
